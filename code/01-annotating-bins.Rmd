...
Find bins that are Sulfitobacter These bins are MAGs (metagenome assembled genomes), and I'll find which are Sulfitobacter by using the CATBAT consensus results of what each contig is. Since I know what microbe each contig is, I'll look for bins where most contigs are Sulfitobacter.
...
classification <- classification[str_detect(classification$genus, "Sulfitobacter"),]

# add this classification file to the repo
write.csv(classification, "../output/CATBAT_sulfitobacter.csv")
```

Now that I have a classification file and my bins, I can find bins where the most reads and contigs are Sulfitobacter.

```{r}
# get Sulfitobacter bins
...
for (i in 2:7){
  ...
  for (binname in bins){
    ...
    consensus <- all_contigs %>%
      ...
      filter(genus != "no support") # only to get all possible Mar bins
    index <- which(str_detect(consensus$genus, "Sulfitobacter"))
    consensus$species[index] <- "Sulfitobacter"
    consensus <- consensus$species[which(consensus$support == max(consensus$support))]
    print(consensus)
    
    if (identical(consensus[str_detect(consensus, "Sulfitobacter")], character(0)) == F){
      a <- a + 1
...
write.csv(sulf_bins, "../output/all_mar_bins.csv")
```
...
path <- "~/Library/CloudStorage/GoogleDrive-jwinter2@uw.edu/Shared drives/Rocap Lab/Project_ODZ_Marinimicrobia_Jordan/Bins/"
...
for (i in 1:nrow(mar_bins)){
  ...
  for (j in 1:nrow(contig_names)){
    ...
    find_contig <- meta %>% filter(str_detect(read_name, contigname))
...
Alluvial plot to check what contigs/reads went where in each binning method.
47 Bin 508: P41300E03
47 Bin 81: Sulfitobacter
24 Bin 133: unknown, 25% complete
...
# choose starting bin
alluvial <- subset(alluvial, X24_sample_bam_bins == "24_sample_bam_bin.133")
