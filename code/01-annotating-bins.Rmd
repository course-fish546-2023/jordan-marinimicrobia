...
classification <- read.csv("~/github/jordan-marinimicrobia/output/CATBAT_marinimicrobia.csv")
...
for (i in 1:6){
  ...
  for (binname in bins){
    ...
    for (i in 1:nrow(contig_names)){
      ...
      find_contig <- lca %>% filter(str_detect(X..ORF, contigname))
