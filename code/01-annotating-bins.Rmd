...
Use BUSCO (which is similar to CheckM to get size, contamination, etc of bins). One of these steps has to be run on my machine (generating the individual fasta files)

```{r}
# Get fasta files for each bin
all <- readDNAStringSet("/Users/jordini/Library/CloudStorage/GoogleDrive-jwinter2@uw.edu/Shared drives/Rocap Lab/Project_ODZ_Marinimicrobia_Jordan/Annotations/1058_P1_2018_585_0.2um_assembly_plus.fna")
bin1_nms <- read.csv("/Users/jordini/Library/CloudStorage/GoogleDrive-jwinter2@uw.edu/Shared drives/Rocap Lab/Project_ODZ_Marinimicrobia_Jordan/Bins/47_sample_bam_bins/47_sample_bam_bin.234", header = F)
test <- str_detect(all@ranges@NAMES, paste(bin1_nms$V1, collapse = "|"))
index <- which(test == T)
test <- all[index]
writeXStringSet(test, "~/Downloads/test.fa", format = "fasta")
```

```{bash}
busco -i INPUTFASTA -m geno -o ~/github/jordan-marinimicrobia/output/busco_outputs -c 8 --auto-lineage
```
